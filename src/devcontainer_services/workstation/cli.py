...
def validate(verbose, debug):
    ...
    try:
        ...
        if overall_issues == 0 and overall_recommendations == 0:
            ...
            console.print(
                "   https://github.com/Troubladore/devcontainer-service-manager/blob/main/docs/optimization-guide.md"
            )
        else:
            console.print(
                f"💡 Consider the {overall_recommendations} recommendations for further optimization"
            )
...
def troubleshoot():
    ...
    try:
        results = validate_workstation_setup(
            debug_mode=False
        )  # Don't overwhelm troubleshoot with debug
        validation = results["validation"]
...
def wsl2_optimize(verbose, debug):
    ...
    if debug:
        logging.basicConfig(level=logging.DEBUG, format="%(levelname)s: %(message)s")
        console.print(
            "🐛 Debug mode enabled - showing detailed WSL2 detection and optimization steps"
        )
    elif verbose:
        logging.basicConfig(level=logging.INFO, format="%(levelname)s: %(message)s")
    ...
    if not optimizer.system_info.get("is_wsl"):
        ...
        if debug or verbose:
            console.print(
                "💡 WSL2 detection failed. Run with --debug to see detection method details"
            )
        return
    ...
    try:
        if apply_all:
            ...
            if docker_buildkit:
                ...
                results["docker_buildkit"] = optimizer.apply_docker_buildkit_optimization(
                    dry_run=dry_run
                )
...
def cleanup():
    ...
    try:
        from ..caching.cleanup import emergency_cleanup, get_resource_usage

        # Show current usage
        usage = get_resource_usage()
        console.print("Current Docker usage:")
