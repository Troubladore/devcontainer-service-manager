...
class PortAllocator:
    ...
    def allocate_port(self, namespace: str, service: str, preferred_port: Optional[int] = None) -> int:
        ...
        if preferred_port and port_range.start <= preferred_port <= port_range.end:
            if self._is_port_available(preferred_port) and preferred_port not in port_range.allocated_ports:
        ...
        raise RuntimeError(f"No available ports in range {port_range.start}-{port_range.end} for namespace {namespace}")
                    ...
                    if len(parts) == 2:
                        try:
                            ...
                            if not self._is_port_available(external_port):
                                conflicts.append(f"Port {external_port} for service {service_name} is already in use")
                            elif external_port in requested_ports:
                                conflicts.append(f"Port {external_port} requested by multiple services")
                            else:
