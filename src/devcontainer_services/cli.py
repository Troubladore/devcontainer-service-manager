...
def up(config: Path, namespace: str | None, dry_run: bool):
    ...
    try:
        ...
        if success_count == total_count:
            console.print(
                f"[bold green]✓ Successfully started {success_count}/{total_count} services[/bold green]"
            )
        else:
            console.print(
                f"[bold yellow]⚠ Started {success_count}/{total_count} services[/bold yellow]"
            )
...
def down(namespace: str | None, all: bool):
...
def _stop_namespace_services(namespace: str):
    ...
    with console.status("[bold yellow]Stopping services..."):
        ...
        try:
            ...
            containers = client.containers.list(
                filters={"label": f"devcontainer-service-manager.namespace={namespace}"}
            )

            for container in containers:
                service_name = container.labels.get(
                    "devcontainer-service-manager.service", "unknown"
                )
                service_pool.services[service_name] = service_pool.ServiceInfo(
                    name=service_name, namespace=namespace, template="", container_id=container.id
                )
    ...
    console.print(
        f"[bold green]✓ Stopped {stopped_count} services in namespace {namespace}[/bold green]"
    )
...
def status(namespace: str | None, all: bool):
    """Show service status."""
    try:
        ns_manager = NamespaceManager()
...
def _show_namespace_status(namespace: str):
    ...
    try:
        ...
        client = docker.from_env()
        containers = client.containers.list(
            all=True, filters={"label": f"devcontainer-service-manager.namespace={namespace}"}
        )
        ...
        for container in containers:
            service_name = container.labels.get(
                "devcontainer-service-manager.service", container.name
            )
...
def clean(unused: bool, force: bool, namespace: str | None):
...
def health(namespace: str | None):
    ...
    try:
        ...
        for service_name, result in namespace_results.items():
            ...
            table.add_row(
                service_name, f"[{status_color}]{result.status.value}[/{status_color}]", check_time
            )
...
def repair(service: str, namespace: str | None):
    ...
    try:
        ...
        console.print(
            f"[bold blue]Repairing service '{service}' in namespace: {namespace}[/bold blue]"
        )
        ...
        with console.status("[bold yellow]Repairing service..."):
...
def template_cmd(action: str, template_name: str | None):
    ...
    try:
        if action == "list":
            ...
            for ns_info in active_namespaces:
                ...
                table.add_row(ns_info.name, ns_info.project, ns_info.branch, str(services_count))
