...
class CrossRepoCacheManager:
    ...
    def pull_from_cache(self, fingerprint: str, image_type: str, local_tag: str) -> bool:
        ...
        try:
            ...
            # Tag as local image
            subprocess.run([
                "docker", "tag", cache_image, local_tag
            ], check=True)
    ...
    def push_to_cache(self, local_tag: str, fingerprint: str, image_type: str) -> bool:
        ...
        try:
            # Tag for cache
            subprocess.run([
                "docker", "tag", local_tag, cache_image
            ], check=True)
...
def smart_docker_build(project_root: Path, 
                      dockerfile: str,
                      image_tag: str, 
                      image_type: str,
                      dependency_files: List[str]) -> bool:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 5:
        print("Usage: fingerprint.py <project_root> <dockerfile> <image_tag> <image_type> [dep_file1] [dep_file2] ...")
        sys.exit(1)
    ...
    success = smart_docker_build(
        project_root, dockerfile, image_tag, image_type, dependency_files
    )
