...
class TestCleanupManager:
    ...
    def cleanup_project(self, project_dir: Path, timeout: int = 60) -> bool:
        ...
        try:
            ...
            result = subprocess.run([
                "docker", "compose",
                "-f", str(compose_file),
                "down", "--volumes", "--remove-orphans", "--timeout", str(timeout // 2)
            ], 
            cwd=project_dir,
            capture_output=True, 
            text=True,
    ...
    def _get_compose_project_name(self, compose_file: Path) -> Optional[str]:
    ...
    def _get_networks_by_name_pattern(self, pattern: str) -> List[str]:
        ...
        try:
            result = subprocess.run([
                "docker", "network", "ls", "--filter", f"name={pattern}",
                "--format", "{{.Name}}"
            ], capture_output=True, text=True)
    ...
    def _get_volumes_by_name_pattern(self, pattern: str) -> List[str]:
        ...
        try:
            result = subprocess.run([
                "docker", "volume", "ls", "--filter", f"name={pattern}",
                "--format", "{{.Name}}"
            ], capture_output=True, text=True)
