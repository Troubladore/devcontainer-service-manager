...
class TestActionableRecommendations:
    ...
    def test_all_recommendations_have_instructions(self):
        ...
        assert len(all_recommendations) > 0, "No recommendations found - test cannot validate actionable instructions"
    ...
    def test_docker_buildkit_recommendation_is_actionable(self):
        ...
        if buildkit_recs:
            ...
            for rec in buildkit_recs:
                ...
                assert "DOCKER_BUILDKIT=1" in rec, f"BuildKit recommendation should include 'DOCKER_BUILDKIT=1': {rec}"
                ...
                assert has_implementation, f"BuildKit recommendation should include implementation steps: {rec}"
    ...
    def test_wsl_filesystem_performance_recommendation_is_actionable(self):
        ...
        with patch.object(optimizer, 'system_info', {
            'is_wsl': True,
            'platform': 'Linux',
            'wsl_version': '2'
        }):
        ...
        if fs_recs:
            for rec in fs_recs:
                ...
                has_implementation = any(element in rec for element in actionable_elements)
                assert has_implementation, f"Filesystem recommendation should include implementation steps: {rec}"
    ...
    def test_resource_recommendations_are_actionable(self):
        ...
        if recommendations:
            for rec in recommendations:
                ...
                has_implementation = any(element in rec for element in actionable_elements)
                assert has_implementation, f"Resource recommendation should include implementation steps: {rec}"
        ...
        if total_recommendations > 0:
            ...
            for rec in docker_recs:
                ...
                for vague in vague_words:
                    assert vague not in rec.lower(), f"Docker recommendation uses vague language '{vague}': {rec}"
