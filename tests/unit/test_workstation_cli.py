...
class TestWorkstationCLI:
    ...
    def test_validate_command_basic(self, mock_optimizer_class):
        ...
        with patch(
            "devcontainer_services.workstation.cli.validate_workstation_setup"
        ) as mock_validate:
...
class TestWorkstationOptimizerDebugMode:
    ...
    def test_optimizer_debug_mode_enabled(self):
        ...
        with patch(
            "devcontainer_services.workstation.setup.WorkstationOptimizer._detect_system_info"
        ) as mock_detect:
...
class TestWSL2DebugDetection:
    ...
    def test_wsl2_detection_all_methods_fail_with_debug(
        self, mock_path_exists, mock_subprocess, mock_environ
    ):
        ...
        with patch(
            ...
            with patch("builtins.print") as mock_print:
                ...
                assert (
                    len(debug_calls) > 6
                )  # Should be more than 6 debug calls (start + 6 methods + end)

                # Check for specific debug messages
                debug_messages = [str(call) for call in debug_calls]
                assert any(
                    "Starting WSL2 detection using 6 different methods" in msg
                    for msg in debug_messages
                )
                assert any(
                    "WSL NOT DETECTED: All 6 detection methods failed" in msg
                    for msg in debug_messages
                )
