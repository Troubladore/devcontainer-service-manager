...
class TestPerformance:
    ...
    def test_dependency_resolution_performance(self, service_pool):
        ...
        for i, service_name in enumerate(ordered_services):
            service_index = int(service_name.split('-')[1])
    ...
    def test_port_allocation_performance(self, port_allocator):
        ...
        for namespace in namespaces:
            ...
            for service_id in range(services_per_namespace):
                ...
                assert port_range.start <= port <= port_range.end, \
        ...
        assert avg_time_per_allocation < 0.01, \
    ...
    def test_health_monitoring_performance(self, health_monitor, temp_config_dir):
        ...
        assert avg_time_per_service < 0.05, \
...
class TestCleanup:
    ...
    def test_service_cleanup_completeness(self, service_pool, mock_docker, sample_services_config):
        ...
        with patch('devcontainer_services.core.service_pool.docker') as mock_docker_module:
            ...
            initial_containers = mock_docker.containers.list(all=True)
            ...
            assert len(service_pool.services) == len(initial_services), \
    ...
    def test_namespace_cleanup_isolation(self, namespace_manager, temp_config_dir, mock_docker):
        ...
        target_containers = [c for c in remaining_containers 
                           if c.labels.get("devcontainer-service-manager.namespace") == target_namespace]
        other_containers = [c for c in remaining_containers 
        ...
        assert len(running_others) == len(namespaces) - 1, \
    ...
    def test_health_monitor_cleanup(self, health_monitor, temp_config_dir):
        ...
        # Verify cleanup
        assert target_namespace not in health_monitor.monitored_pools, \
               "Target namespace should be removed from monitoring"
        assert namespaces[1] in health_monitor.monitored_pools, \
        ...
        assert health_monitor._monitor_thread is None or not health_monitor._monitor_thread.is_alive(), \
    ...
    def test_graceful_shutdown_cleanup(self, service_pool, health_monitor, mock_docker,
                                     sample_services_config, temp_config_dir):
        ...
        with patch('devcontainer_services.core.service_pool.docker') as mock_docker_module:
            ...
            running_containers = [c for c in mock_docker.containers.list() 
                                if c.status == 'running']
            ...
            final_running = [c for c in mock_docker.containers.list() 
                           if c.status == 'running']
    ...
    def test_cleanup_error_recovery(self, service_pool, mock_docker, sample_services_config):
        ...
        with patch('devcontainer_services.core.service_pool.docker') as mock_docker_module:
            ...
            non_postgres_services = [name for name in stop_results 
