        ...
        with patch("pathlib.Path.home", return_value=Path(self.temp_dir)):
            ...
            rollback_content = "\n".join(
                [
                    ...
                    if "DOCKER_BUILDKIT" not in line
                    and "COMPOSE_DOCKER_CLI_BUILD" not in line
                    and "added by dcm-setup" not in line
                ]
            )
...
class TestWSLConfigOptimization:
    ...
    def test_apply_wsl_config_fresh_install(self, mock_detect):
        ...
        with patch("os.path.exists") as mock_exists, patch("os.listdir") as mock_listdir, patch(
            "os.path.isdir"
        ) as mock_isdir, patch("os.path.getsize") as mock_getsize, patch(
            "subprocess.run"
            ...
            assert any(
                "/mnt/c/Users/testuser/.wslconfig" in str(call)
    ...
    def test_wsl_config_multiple_users_debug(
        self, mock_file, mock_subprocess, mock_isdir, mock_listdir, mock_exists, mock_detect
    ):
        ...
        mock_isdir.side_effect = lambda path: any(
            user in path for user in ["alice", "bob", "Public", "Default"]
        )
...
class TestApplyAllSafeOptimizations:
    ...
    def test_apply_all_safe_optimizations_success(self, mock_detect):
        ...
        with patch.object(
            optimizer, "apply_docker_buildkit_optimization", return_value=True
        ) as mock_docker, patch.object(
            optimizer, "apply_filesystem_migration_helper", return_value=True
        ) as mock_fs, patch.object(
            optimizer, "apply_disk_cleanup_optimization", return_value=True
        ) as mock_disk, patch.object(
            optimizer, "apply_wsl_config_optimization", return_value=True
        ) as mock_wsl:
    ...
    def test_apply_all_safe_optimizations_partial_failure(self, mock_detect):
        ...
        with patch.object(
            ...
            optimizer, "apply_filesystem_migration_helper", return_value=False
        ), patch.object(
            optimizer, "apply_disk_cleanup_optimization", return_value=True
        ), patch.object(
            optimizer, "apply_wsl_config_optimization", return_value=True
