...
from devcontainer_services.core.namespace_manager import NamespaceManager, NamespaceInfo
from devcontainer_services.core.service_pool import ServicePool, ServiceInfo, ServiceStatus
from devcontainer_services.core.port_allocator import PortAllocator, PortRange
...
class MockContainerManager:
    ...
    def list(self, all: bool = False, filters: Optional[Dict] = None) -> List[MockDockerContainer]:
        containers = []
...
def services_config_file(temp_config_dir, sample_services_config):
    ...
    config_content = {
        ...
        "services": sample_services_config
    }
...
def verify_no_port_conflicts(port_allocator: PortAllocator, services_config: Dict[str, Any]) -> bool:
...
def verify_proper_dependencies(service_pool: ServicePool) -> bool:
    ...
    for service_name, service_info in service_pool.services.items():
        for dependency in service_info.depends_on:
            ...
            if dep_service.status == ServiceStatus.MISSING and service_info.status == ServiceStatus.HEALTHY:
                return False
