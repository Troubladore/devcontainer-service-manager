...
def docker_client():
    ...
    try:
        containers = client.containers.list(
            all=True, filters={"label": "devcontainer-service-manager.test=true"}
        )
...
class TestRealDockerIntegration:
    ...
    def test_postgres_container_lifecycle(
        self, docker_client, integration_service_pool, integration_namespace, port_allocator
    ):
        """Test complete PostgreSQL container lifecycle."""
        # Setup service configuration
        postgres_config = {"template": "postgres:16", "persistent": True, "health_check": True}

        service_name = "postgres"
        integration_service_pool.add_service(service_name, postgres_config)

        # Allocate ports
        port_allocator.allocate_port_range(integration_namespace)
        port_allocator.allocate_port(integration_namespace, service_name)
        ...
        try:
            ...
            containers = docker_client.containers.list(
                filters={"label": f"devcontainer-service-manager.namespace={integration_namespace}"}
            )
    ...
    def test_multi_service_dependency_resolution(
        self, docker_client, integration_service_pool, integration_namespace, port_allocator
    ):
        ...
        services_config = {
            ...
            "redis": {"template": "redis:7-alpine", "persistent": True, "health_check": True},
    ...
    def test_port_conflict_detection_real(
        self, docker_client, integration_namespace, port_allocator
    ):
        ...
        try:
            ...
            services_config = {"web-server": {"ports": [f"{conflicting_port}:80"]}}
            ...
            assert any(
                str(conflicting_port) in conflict for conflict in conflicts
    ...
    def test_service_recovery_real(
        self, docker_client, integration_service_pool, integration_namespace
    ):
        """Test service recovery with real containers."""
        # Setup service
        redis_config = {"template": "redis:7-alpine", "persistent": True, "health_check": True}
    ...
    def test_health_monitoring_real(
        self, docker_client, integration_service_pool, integration_namespace
    ):
        ...
        try:
            ...
            while wait_time < max_wait:
                ...
                if (
                    integration_namespace in health_results
                    and service_name in health_results[integration_namespace]
                ):
    ...
    def test_namespace_isolation_real(self, docker_client, temp_config_dir, port_allocator):
        ...
        redis_config = {"template": "redis:7-alpine", "persistent": True}
        ...
        try:
            ...
            containers1 = docker_client.containers.list(
                filters={"label": f"devcontainer-service-manager.namespace={namespace1}"}
            )
            containers2 = docker_client.containers.list(
                filters={"label": f"devcontainer-service-manager.namespace={namespace2}"}
            )
    ...
    def test_full_workflow_integration(
        ...
        try:
            ...
            containers_after_restart = docker_client.containers.list(
                filters={"label": f"devcontainer-service-manager.namespace={integration_namespace}"}
            )
            ...
            time.sleep(2)
            final_containers = docker_client.containers.list(
                filters={"label": f"devcontainer-service-manager.namespace={integration_namespace}"}
            )

            # Persistent containers should exist but be stopped
            persistent_services = [
                name for name, config in services_config.items() if config.get("persistent", True)
            ]
            assert len(final_containers) >= len(
                persistent_services
...
class TestCLIIntegration:
    ...
    def test_cli_up_command_integration(
        self, docker_client, temp_config_dir, integration_namespace
    ):
        ...
        try:
            ...
            result = runner.invoke(
                main, ["up", "--config", str(config_file), "--namespace", integration_namespace]
            )
            ...
            status_result = runner.invoke(main, ["status", "--namespace", integration_namespace])

            assert (
                status_result.exit_code == 0
            ), f"CLI status command failed: {status_result.output}"
            assert "redis" in status_result.output, "Status should show redis service"

        finally:
            # Cleanup using CLI
            runner = CliRunner()
            cleanup_result = runner.invoke(main, ["down", "--namespace", integration_namespace])
