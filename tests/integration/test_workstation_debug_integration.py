...
class TestWorkstationDebugIntegration:
    ...
    def test_validate_debug_output_format(self):
        ...
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            cwd="/home/eru-admin/repos/devcontainer-service-manager",
        )
        ...
        wsl_detection_duplicates = [
            line
            for line in plain_debug_lines
            if "Starting WSL2 detection" in line or "Method" in line
        ]
        assert (
            len(wsl_detection_duplicates) == 0
        ), f"Found duplicate WSL detection debug lines: {wsl_detection_duplicates}"
    ...
    def test_wsl2_optimize_debug_output_format(self):
        ...
        assert (
            "This command only works in WSL environments" in output
            or "WSL2 optimizations applied" in output
        )
    ...
    def test_wsl_version_detection_robustness(self):
        ...
        with patch("builtins.open", mock_open(read_data=mock_proc_version)):
            with patch.dict(os.environ, {"WSL_DISTRO_NAME": "Ubuntu"}):
                ...
                # Should detect WSL
                assert optimizer.system_info["is_wsl"]
    ...
    def test_debug_output_no_sensitive_information_leak(self):
        ...
        for pattern in sensitive_patterns:
            ...
            ), f"Debug output contains potentially sensitive pattern: {pattern}"
