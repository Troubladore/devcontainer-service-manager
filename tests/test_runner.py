...
class TestRunner:
    ...
    def run_contract_tests(self) -> Dict[str, Any]:
        ...
        contract_args = [
            ...
            "-k", "contract",  # Focus on contract tests
        ]
    ...
    def generate_summary(self):
        ...
        passed_tests = len([k for k, v in self.results.items() 
                          if k.endswith('_tests') and v.get('passed', False)])
        ...
        # Overall result
        all_passed = all(v.get('passed', False) for k, v in self.results.items() 
                        if k.endswith('_tests'))
...
def main():
    ...
    parser.add_argument("--category", choices=[
        "unit", "integration", "contract", "state", "performance", "edge", "all"
    ], default="all", help="Test category to run")
    parser.add_argument("--validate-contracts", action="store_true",
                       help="Validate service manager contracts")
    ...
    if args.category != "all":
        ...
        all_passed = all(v.get('passed', False) for k, v in runner.results.items() 
                        if k.endswith('_tests'))
